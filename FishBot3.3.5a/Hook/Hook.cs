        ...
        public bool InstallHook()
        {
            try
            {
                ...
                if (Memory.IsProcessOpen)
                {
                    ...
                    string sBytes = "";
                    foreach(byte b in _endSceneOriginalBytes)
                    {
                        sBytes += b.ToString() + ", ";
                    }
                    Log.Debug("Original EndSceneBytes = ({0})", sBytes);
            ...
            lock (_executeLockObject)
            {
                ...
                if (Memory.IsProcessOpen && Installed)
                {
                    ...
                    try
                    {
                        ...
                        //    IntPtr dwAddress = Memory.Read<IntPtr>(_retnInjectionAsm);
                        //    if (dwAddress.ToInt32() != 0)
                        //    {
                        //        //Log.Write("dwAddress {0}", dwAddress);
                        //        byte buf = Memory.Read<byte>(dwAddress);
                        //        while (buf != 0)
                        //        {
                        //            retnByte.Add(buf);
